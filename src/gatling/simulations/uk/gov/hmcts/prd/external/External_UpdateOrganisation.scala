...
object External_UpdateOrganisation {
  ...
  val UpdateOrgMin = config.getString("external.updateOrgMin").toInt

  val UpdateOrgMax = config.getString("external.updateOrgMax").toInt
  ...
  val updateOrganisation = exec(_.setAll(
    ...
    .exec(http("RD02_Internal_ActivateOrganization")
      .put("/refdata/internal/v1/organisations/${NewPendingOrg_Id}") //AQO5KI8
